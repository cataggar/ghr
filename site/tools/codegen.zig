...
fn scanDir(alloc: std.mem.Allocator, entries: *std.ArrayList([]u8), dir: []const u8) !void {
    ...
    while (try walker.next(runtime.io)) |entry| {
        ...
        // Normalize Walker's OS-native separator (backslash on Windows) to
        // '/' so the path is safe both as a Zig @import string literal (a
        // bare backslash is an invalid escape) and as a URL.
        if (std.fs.path.sep != '/') {
            for (full) |*c| {
